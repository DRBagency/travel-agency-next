...
const ImageSlider = ({ clienteId, primaryColor }: ImageSliderProps) => {
  ...
  return (
    <section className="relative bg-slate-900 text-white py-20 md:py-28 overflow-hidden">
      ...
      <GradientMesh primaryColor={accentColor} opacity={0.2} />

      {/* Pulsing ring decorations */}
      <div className="absolute top-10 end-10 w-32 h-32 rounded-full border-2 opacity-20 animate-pulse-ring" style={{ borderColor: accentColor }} />
      <div className="absolute bottom-20 start-10 w-24 h-24 rounded-full border-2 opacity-15 animate-pulse-ring" style={{ borderColor: accentColor, animationDelay: "1.5s" }} />
      ...
      <div
        ref={sliderRef}
        className="flex gap-6 overflow-x-auto no-scrollbar slider-snap ps-4 md:ps-[max(1rem,calc((100vw-1280px)/2+1rem))] pe-4 relative z-10"
      >
        {withImages.map((d) => (
          <div
            key={d.id}
