      ...
      <div className="px-3 pb-2.5">
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-4 text-center">
            <Mail className="w-6 h-6 text-drb-turquoise-300 dark:text-drb-turquoise-600 mb-1" />
            <p className="text-xs text-gray-400 dark:text-white/40">{labels.noMessages}</p>
          </div>
        ) : (
          <div className="space-y-1">
            {messages.slice(0, 3).map((msg, i) => (
              <motion.div
                ...
                className={`flex items-start gap-2 rounded-lg px-2.5 py-1.5 border transition-colors ${
                  msg.read
              ...
              >
                <div className="w-6 h-6 rounded-full bg-gradient-to-br from-drb-turquoise-400 to-drb-turquoise-600 flex items-center justify-center text-white text-[9px] font-semibold shrink-0">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-gray-600 dark:text-white/60 line-clamp-1 mt-0.5">
                    {msg.message}
                  </p>
                </div>

                {!msg.read && (
                  <div className="w-1.5 h-1.5 rounded-full bg-drb-turquoise-500 shrink-0 mt-1.5" />
                )}
              </motion.div>
            ))}
          </div>
