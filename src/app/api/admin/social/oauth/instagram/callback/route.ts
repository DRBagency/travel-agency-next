...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const { access_token, token_expires_at, user_id } =
      await exchangeInstagramCode(code);

    // Fetch profile and media
    const [profile, posts] = await Promise.all([
      fetchInstagramProfile(access_token),
      fetchInstagramMedia(access_token),
    ]);
    ...
    return NextResponse.redirect(
      `${baseUrl}/admin/social?success=instagram`
    );
  } catch (err) {
    console.error("Instagram OAuth callback error:", err);
    return NextResponse.redirect(
      `${baseUrl}/admin/social?error=instagram_failed`
    );
  }
}
